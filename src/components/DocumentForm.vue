...
const handleSubmit = async () => {
  ...
  try {
    let medium = documentType.value;
    let userText = encodeURIComponent(prompt.value);
    ...
    const response = await fetch(
      `https://api.marvelly.com.ng/v1/${medium}?text=${userText}`
    );
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <div>
        ...
        <select v-model="documentType" class="w-full px-4 py-2 rounded-lg bg-black/50 border border-green-900 focus:outline-none focus:border-green-500 transition-colors text-white">
          ...
          <option value="pdfv2">PDF v2(Stable Recommended)</option>
